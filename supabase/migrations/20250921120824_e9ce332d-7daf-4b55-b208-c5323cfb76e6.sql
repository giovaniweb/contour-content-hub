...
BEGIN
  ...
  FOR r IN (
    SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'perfis'
  ) LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.perfis;', r.policyname);
  END LOOP;
  -- Drop all policies on public.superusers
  FOR r IN (
    SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'superusers'
  ) LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.superusers;', r.policyname);
  END LOOP;
END $$;
