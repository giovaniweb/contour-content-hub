-- AI Knowledge and Copilot schema (retry without IF NOT EXISTS on policies)
-- 1) Tables
...
CREATE POLICY "Read knowledge sources by access"
ON public.ai_knowledge_sources
FOR SELECT
USING (
  is_public = true OR
...
CREATE POLICY "Read knowledge chunks by source access"
ON public.ai_knowledge_chunks
FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "Admins manage transcripts"
ON public.ai_lesson_transcripts
FOR ALL
USING (auth.uid() IN (SELECT perfis.id FROM public.perfis WHERE perfis.role = 'admin'))
WITH CHECK (auth.uid() IN (SELECT perfis.id FROM public.perfis WHERE perfis.role = 'admin'));

CREATE POLICY "Read transcripts by source access"
ON public.ai_lesson_transcripts
FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "Users manage own copilot sessions"
...
CREATE POLICY "Read messages if own session"
...
CREATE POLICY "Insert messages if own session"
ON public.copilot_messages
FOR INSERT
WITH CHECK (
  EXISTS (
