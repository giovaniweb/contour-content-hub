...
-- First, update the role constraint to include 'user' role
ALTER TABLE public.perfis 
DROP CONSTRAINT IF EXISTS perfis_role_check;

ALTER TABLE public.perfis 
ADD CONSTRAINT perfis_role_check 
CHECK (role = ANY (ARRAY['user'::text, 'cliente'::text, 'admin'::text, 'operador'::text, 'gerente'::text, 'consultor'::text, 'superadmin'::text]));
