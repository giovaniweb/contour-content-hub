...
-- 1. Primeiro, verificar se há dados importantes em documentos_tecnicos que precisam ser migrados
-- (assumindo que a migração já foi feita, vamos apenas documentar e deprecated a tabela)

-- 2. Adicionar comentário de depreciação à tabela
...
-- 3. Desabilitar triggers da tabela depreciada para evitar efeitos colaterais
DROP TRIGGER IF EXISTS handle_document_content_change ON public.documentos_tecnicos;

-- 4. Remover políticas RLS da tabela depreciada (mantemos apenas estrutura para compatibilidade temporária)
...
-- 5. Melhorar as políticas RLS para unified_documents (garantir que admin tem acesso total)
...
-- 6. Adicionar índices para performance no sistema novo
...
-- 7. Garantir que triggers estão funcionando corretamente para unified_documents
...
COMMENT ON MIGRATION IS 'Depreciação do sistema documentos_tecnicos e melhorias no unified_documents para migração completa ao novo sistema';