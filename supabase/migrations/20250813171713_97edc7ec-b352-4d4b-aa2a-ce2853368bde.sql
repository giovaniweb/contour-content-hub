...
-- Garantir que apenas usuários autenticados vejam seus próprios perfis
-- Estas políticas já existem, mas vamos reforçar:
...
CREATE POLICY IF NOT EXISTS "Authenticated users can view equipment" 
...
CREATE POLICY IF NOT EXISTS "Authenticated users can view technical documents" 
...
CREATE POLICY IF NOT EXISTS "Only admins can view gpt config" 
...
CREATE POLICY IF NOT EXISTS "Only admins can manage gpt config" 
ON public.gpt_config 
FOR ALL 
TO authenticated
USING (public.is_admin())
...
-- 5. Verificar se existe alguma view problemática oculta
-- Dropar e recriar a view public que pode estar causando problema
