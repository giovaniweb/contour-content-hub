...
const Videos: React.FC = () => {
  ...
  useEffect(() => {
    const loadLikesCount = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('favoritos').select('video_id').in('video_id', videos.map(v => v.id));
      if (!error && data) {
  ...
  const filteredVideos = useMemo(() => {
    return videos.filter(video => {
      ...
      const matchesEquipment = !selectedEquipment || selectedEquipment === 'all' || video.categoria === selectedEquipment;
  ...
  return <AuroraPageLayout>
      <StandardPageHeader icon={Video} title="Biblioteca de Vídeos" subtitle="Explore nossa coleção completa de vídeos com procedimentos e equipamentos" statusBadges={statusBadges} />
      ...
      <div className="container mx-auto px-6 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
              ...
              <Select value={selectedEquipment} onValueChange={setSelectedEquipment}>
                ...
                <SelectContent className="bg-slate-900 border-cyan-400/30">
                  ...
                  {equipments.map(equipment => <SelectItem key={equipment.id} value={equipment.nome} className="text-white">
                      {equipment.nome}
                    </SelectItem>)}
                </SelectContent>
              </Select>
        ...
        {/* Error state */}
        {error && <div className="aurora-glass rounded-2xl p-6 border-red-500/30">
        ...
        {isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.from({
          length: 8
        }).map((_, index) => <div key={index} className="aurora-glass rounded-2xl p-4 animate-pulse">
            ...
            {filteredVideos.map(video => <div key={video.id} className="aurora-card rounded-2xl group hover:scale-105 transition-all duration-300">
                {/* Selection Checkbox */}
                {video.url_video}
                ...
                <div className="relative aspect-video bg-slate-800/50 overflow-hidden rounded-t-2xl cursor-pointer" onClick={() => handleVideoPlay(video)}>
                  {video.thumbnail_url ? <img src={video.thumbnail_url} alt={video.titulo} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900">
                  ...
                  {/* Duration */}
                  {video.duracao && <div className="absolute bottom-3 right-3 aurora-glass px-2 py-1 rounded-lg text-xs text-cyan-400">
                      {video.duracao}
                    </div>}
                </div>
                ...
                <div className="p-4">
                  ...
                  {video.descricao_curta && <p className="text-xs text-slate-400 mb-3 line-clamp-2">
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400 mb-3">
                    <div className="flex flex-col">
                      ...
                      {video.categoria && <span className="text-cyan-400 font-medium">{getEquipmentName(video.categoria)}</span>}
                    </div>
                    <span>{video.downloads_count || 0} downloads</span>
                  </div>

                  {/* Tags */}
                  {video.tags && video.tags.length > 0 && <div className="flex flex-wrap gap-1 mb-4">
                      {video.tags.slice(0, 2).map((tag, index) => <Badge key={index} variant="secondary" className="text-xs bg-cyan-400/20 text-cyan-400 border-cyan-400/30">
                          {tag}
                        </Badge>)}
                      {video.tags.length > 2 && <Badge variant="outline" className="text-xs border-cyan-400/30 text-cyan-400">
                          +{video.tags.length - 2}
                        </Badge>}
                    </div>}
                  ...
                  {/* Actions */}
                  <div className="flex items-center gap-2">
                    <Button size="sm" onClick={() => handleVideoPlay(video)} className="aurora-button rounded-xl flex-1">
                      <Play className="h-4 w-4 mr-1" />
                      Assistir
                    </Button>
                    
                    <Button size="sm" variant="outline" onClick={e => handleLike(video.id, e)} className={`rounded-xl border-cyan-400/30 ${likedVideos.has(video.id) ? 'bg-cyan-400/20 text-cyan-400 border-cyan-400' : 'text-cyan-400 hover:bg-cyan-400/20'}`}>
                      <Heart className={`h-4 w-4 mr-1 ${likedVideos.has(video.id) ? 'fill-current' : ''}`} />
                      {likesCount[video.id] || 0}
                    </Button>
                    
                    {video.url_video && <Button size="sm" variant="outline" onClick={e => handleSingleDownload(video.url_video!, video.titulo, e)} disabled={isDownloading} className="rounded-xl border-cyan-400/30 text-cyan-400 hover:bg-cyan-400/20">
            ...
            {filteredVideos.map(video => <div key={video.id} className="aurora-card rounded-2xl hover:scale-[1.02] transition-all duration-300">
                <div className="p-4">
                  <div className="flex items-center gap-4">
                    {/* Selection Checkbox */}
                    {video.url_video && <Checkbox checked={selectedVideos.has(video.id)} onCheckedChange={() => toggleVideoSelection(video.id)} className="bg-black/50 border-cyan-400/50" />}
                    
                    {/* Thumbnail */}
                    <div className="relative w-32 h-18 bg-slate-800/50 rounded-xl overflow-hidden flex-shrink-0 cursor-pointer" onClick={() => handleVideoPlay(video)}>
                      {video.thumbnail_url ? <img src={video.thumbnail_url} alt={video.titulo} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {video.descricao_curta && <p className="text-sm text-slate-400 mb-2 line-clamp-2">
                          {video.descricao_curta}
                        </p>}
                    ...
                    {/* Actions */}
                    <div className="flex items-center gap-2">
                      <Button size="sm" onClick={() => handleVideoPlay(video)} className="aurora-button rounded-xl">
                        <Play className="h-4 w-4 mr-1" />
                        Assistir
                      </Button>
        ...
        {/* Empty state */}
        {filteredVideos.length === 0 && !isLoading && <div className="text-center py-16">
            <div className="aurora-glass rounded-2xl p-12 max-w-md mx-auto">
              ...
              <p className="text-slate-400 mb-6">
                {searchTerm || selectedEquipment || selectedProcedure ? 'Tente ajustar seus filtros de busca' : 'Ainda não há vídeos disponíveis'}
              </p>
            </div>
          </div>}

        {/* Video Player */}
        <UserVideoPlayer video={selectedVideo} open={isPlayerOpen} onOpenChange={setIsPlayerOpen} />
      </div>
    </AuroraPageLayout>;
};
