...
const Videos: React.FC = () => {
  ...
  const selectAllVideos = () => {
    const videosWithUrl = videos.filter(v => v.url_video).map(v => v.id);
            ...
            {videos.map(video => (
              <div key={video.id} className="bg-slate-800/50 border-2 border-slate-700/50 hover:border-cyan-400/50 overflow-hidden transition-all duration-300 group hover:shadow-lg hover:shadow-cyan-400/10 rounded-2xl">
                ...
                <div className="relative aspect-video bg-slate-800/50 overflow-hidden rounded-t-2xl cursor-pointer" onClick={() => handleVideoPlay(video)}>
                  {video.thumbnail_url ? (
                    <img src={video.thumbnail_url} alt={video.titulo} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                  ) : (
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-700/30">
                    ...
                    <Button 
                      ...
                      className={`rounded-xl border-cyan-400/30 ${likedVideos.has(video.id) ? 'bg-cyan-400/20 text-cyan-400 border-cyan-400' : 'text-cyan-400 hover:bg-cyan-400/20'}`}
                    >
