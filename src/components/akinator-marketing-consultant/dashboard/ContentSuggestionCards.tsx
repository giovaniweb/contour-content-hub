...
const ContentSuggestionCards: React.FC<ContentSuggestionCardsProps> = ({ state, diagnostic }) => {
  ...
  const getFormatIcon = (format: string) => {
    switch (format) {
      ...
      case 'reels': return <PlayCircle className="h-4 w-4" />;
      case 'carrossel': return <Image className="h-4 w-4" />;
      case 'story': return <Users className="h-4 w-4" />;
  ...
  const getFormatColor = (format: string) => {
    switch (format) {
      case 'vídeo': return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
      case 'reels': return 'bg-purple-500/20 text-purple-400 border-purple-500/30';
      case 'carrossel': return 'bg-green-500/20 text-green-400 border-green-500/30';
      case 'story': return 'bg-orange-500/20 text-orange-400 border-orange-500/30';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold aurora-heading flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-aurora-electric-purple" />
            Sugestões Inteligentes de Conteúdo
          </h3>
          <p className="text-sm aurora-body opacity-70 mt-1">
