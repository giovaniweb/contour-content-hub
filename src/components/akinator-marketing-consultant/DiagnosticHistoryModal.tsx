...
import { History, Clock, Trash2, Eye, Download, AlertCircle, Calendar, FileText } from "lucide-react";
  ...
  return <Dialog>
      <DialogTrigger asChild>
        <Button variant="outline" className="flex items-center gap-2">
          ...
          {savedDiagnostics.length > 0 && <Badge variant="secondary" className="ml-1">
              {savedDiagnostics.length}
            </Badge>}
        </Button>
      </DialogTrigger>
      
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-fluida-lightGray">
        <DialogHeader>
        ...
        <div className="space-y-6">
              ...
              <Card className="aurora-card border-blue-500/30">
                ...
                <CardContent className="pt-0">
                  <div className="flex gap-2">
                    {currentSession.isCompleted && <Button onClick={() => handleLoadDiagnostic(currentSession)} size="sm" className="flex items-center gap-1">
          ...
          {savedDiagnostics.length > 0 && <div>
              ...
              <div className="grid gap-4">
                {savedDiagnostics.map((session, index) => <motion.div key={session.id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <CardTitle className="text-white flex items-center gap-2">
                              ...
                              {session.id === currentSession?.id && <Badge variant="outline" className="border-blue-500/30 text-blue-400">
                                  Atual
                                </Badge>}
                            </CardTitle>
                            <p className="text-sm text-white/70 mt-1">
          ...
          {/* Estado vazio */}
          {!currentSession && savedDiagnostics.length === 0 && <div className="text-center py-12">
